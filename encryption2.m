function [ cip,dnkey,ext_val,TT,T2 ] = encryption2( im,cover,key,def,MC )
% 三通道分别压缩，合并一起，直方图移位，置乱，有意义密文嵌入
...
aa = zeros(256,1);
for x_value = 0:255
    aa(x_value+1)= sum(T2(:)==x_value);
end
[~,index] = max (aa);
TT = index-MC;
T3 = mod(T2-TT,256);
% 嵌入
[ cip ] = emb3( T3,cover,def,x3,y3 );
...
a2=mod(a2*sha_sum,11)+20;b2=mod(b2*sha_sum,0.11)+0.89;x02=x02*sha_sum/8192;y02=y02*sha_sum/8192;
a3=mod(a3*sha_sum,11)+20;b3=mod(b3*sha_sum,0.11)+0.89;x03=x03*sha_sum/8192;y03=y03*sha_sum/8192;
